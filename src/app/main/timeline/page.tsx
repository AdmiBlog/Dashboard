...
function BBItem({item,deleteHandler,saveHandler,pasteHandler,uploadWithTip,updated}:{item:BB,deleteHandler:()=>void,saveHandler:(content:string)=>void,pasteHandler:(editorRef:RefObject<AceEditor>,event:ClipboardEvent)=>void,uploadWithTip:(file:File)=>Promise<string>,updated:number}):ReactElement{
  ...
  return (
    <div className="times-item">
      <div className="times-item-topbar">
        <div className="times-item-mainbar" style={{display:editing?"none":"block"}}>
          <Button
            icon={<ComposeRegular/>}
            className="times-item-mainbar-editbtn"
            size="small"
          ...
          <Button
            icon={<DeleteRegular/>}
            className="times-item-mainbar-delbtn"
            style={{color:"red"}}
            size="small"
            title="删除"
        ...
        <div className="times-item-editbar" style={{display:editing?"flex":"none"}}>
          <Button
          ...
          <Button
            ...
            title="取消"
          />
          <span className="times-item-editbar-separator"/>
          <input
            type="file"
...
function NewBBItem({item,saveHandler,pasteHandler,uploadWithTip}:{item:BB,saveHandler:(content:string)=>void,pasteHandler:(editorRef:RefObject<AceEditor>,event:ClipboardEvent)=>void,uploadWithTip:(file:File)=>Promise<string>}){
  ...
  return (editing?
    <div className="times-item">
      <div className="times-item-topbar">
        <div className="times-item-editbar">
          <Button
            icon={<CheckmarkRegular/>}
            size="small"
            title="保存"
...
export default function Page(){
  ...
  const [timeCount,setTimelineCount]=useState(-1);
  const [cols,setCols]=useState(1);
  const [updated,setUpdated]=useState(0);
  const messageBarRef=useRef<MessagesRef>(null);
  const searchParams=useSearchParams();
  const page=searchParams.get("page")?parseInt(searchParams.get("page")!):1;
  const maxPage=Math.ceil(timeCount/30);
  ...
  useEffect(()=>{(async ()=>{
    setTimelineCount(await getTimelineCount());
    const times=await getTimelines((page-1)*30,page*30);
    if(times){
      setBBContent(times.map(item=>{
        const deleteHandler=()=>{
          setDialogState({
            open:true,
            title:"删除说说",
            ...
            onConfirm:async ()=>{
              ...
              if(await removeTimelines(item.time)){
  ...
  return (
    <>
      ...
      <div id="times-list">
        <wc-waterfall id="times-waterfall" gap={10} cols={cols}>
          {bbContent}
        </wc-waterfall>
      </div>
      <div id="times-pagination">
        {page-2>0?<Link className="times-pagination-pgbtn" href={`/main/times?page=1`}>
          <Button appearance="secondary" shape="circular">1</Button>
        </Link>:<></>}
        {page-3>0?<Label className="times-pagination-spec">...</Label>:<></>}
        {page-1>0?<Link className="times-pagination-pgbtn" href={`/main/times?page=${page-1}`}>
          <Button appearance="secondary" shape="circular">{page-1}</Button>
        </Link>:<></>}
        <Link className="times-pagination-pgbtn current" href={`/main/times?page=${page}`}>
          <Button appearance="primary" shape="circular">{page}</Button>
        </Link>
        {page+1<=maxPage?<Link className="times-pagination-pgbtn" href={`/main/times?page=${page+1}`}>
          <Button appearance="secondary" shape="circular">{page+1}</Button>
        </Link>:<></>}
        {page+3<=maxPage?<Label className="times-pagination-spec">...</Label>:<></>}
        {page+2<=maxPage?<Link className="times-pagination-pgbtn" href={`/main/times?page=${maxPage}`}>
          <Button appearance="secondary" shape="circular">{maxPage}</Button>
        </Link>:<></>}
        <Label id="times-pagination-text">{`20 条/页 共 ${timeCount} 条`}</Label>
      </div>
