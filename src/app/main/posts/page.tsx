  ...
  return (
    <>
        ...
        (e)=>{
          if(e.key=="Enter"){
            ...
            if(e.currentTarget.value.trim()!=query)
              router.push(`/main/posts/?query=${e.currentTarget.value.trim()}&page=1`);
          }
        }
      }/>
      <div id="posts-topbar">
            ...
            {rows.map(({item:post})=>(
              <TableRow key={post.slug}>
                <TableCell>
                  <TableCellLayout>
                    <Link 
                      href={`/main/posts/edit?slug=${post.slug}&type=post`} 
                ...
                <TableCell>
                  ...
                  <Button 
                      ...
                      onClick={
                        ()=>{
                          router.push(`/main/posts/edit/?slug=${post.slug}&type=post`)
                        }
                      }
                    />
                    <Button 
                        ...
                        setDialogState({
                          ...
                          onConfirm:async ()=>{
                            ...
                            const success=()=>{
                              ...
                              setTimeout(()=>router.push("/main/drafts"),1000);
                            }
      ...
      <div id="posts-pagination">
        {page-2>0?<Link className="posts-pagination-pgbtn" href={`/main/posts?page=1&query=${query}`}>
          <Button appearance="secondary" shape="circular">1</Button>
        </Link>:<></>}
        {page-3>0?<Label className="posts-pagination-spec">...</Label>:<></>}
        {page-1>0?<Link className="posts-pagination-pgbtn" href={`/main/posts?page=${page-1}&query=${query}`}>
          <Button appearance="secondary" shape="circular">{page-1}</Button>
        </Link>:<></>}
        <Link className="posts-pagination-pgbtn current" href={`/main/posts?page=${page}&query=${query}`}>
          <Button appearance="primary" shape="circular">{page}</Button>
        </Link>
        {page+1<=maxPage?<Link className="posts-pagination-pgbtn" href={`/main/posts?page=${page+1}&query=${query}`}>
          <Button appearance="secondary" shape="circular">{page+1}</Button>
        </Link>:<></>}
        {page+3<=maxPage?<Label className="posts-pagination-spec">...</Label>:<></>}
        {page+2<=maxPage?<Link className="posts-pagination-pgbtn" href={`/main/posts?page=${maxPage}&query=${query}`}>
